...
export class BankBasePrismaSource extends AbstractSourceEx<IBankPrismaSchemaType, IBankSourceSchemaType> {
    ...
    async runFetch({filter}: IBankSourceSchemaType["Query"]): Promise<IBankSourceSchemaType["Entity"]> {
    ...
    async runPatchBy({patch, filter}: ISource.IPatchBy<IBankSourceSchemaType>): Promise<unknown> {
 ...
 */
export const $leight_yjqilmsb4ecdcnlew9f5nw2r = true;