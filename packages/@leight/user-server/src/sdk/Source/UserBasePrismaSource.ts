...
export class UserBasePrismaSource extends AbstractSourceEx<IUserPrismaSchemaType, IUserSourceSchemaType> {
    ...
    async runFetch({filter}: IUserSourceSchemaType["Query"]): Promise<IUserSourceSchemaType["Entity"]> {
    ...
    async runPatch({patch, filter}: ISource.IPatch<IUserSourceSchemaType>): Promise<IUserSourceSchemaType["Entity"]> {
 ...
 */
export const $leight_zttwziil3qhuwfaxmd2uiup5 = true;